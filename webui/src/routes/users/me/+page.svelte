<script lang="ts">
	...
	onMount(async ()=>{
        ...
        providers = response.data.result
            .filter((e:ServiceProvider)=>{return !details.providers.find((f)=>{return f==e.abbreviation})})
            .sort((a:ServiceProvider,b:ServiceProvider) => (a.name > b.name) ? 1 : ((b.name > a.name) ? -1 : 0))
		response = await axios.get('/requests/membership', {
			baseURL: $api.base_url+$api.modules.aai
		})
		if(response.status===200) {
			openRequests = response.data.filter((e: MembershipRequest)=>{return e.mail===details.email})
		}
	})
</script>
