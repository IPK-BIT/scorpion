<script lang="ts">
    ...
    let categories: string[] = ["Databases", "Webapplications", "Tools", "Workflows", "Libraries", "Support"]
    ...
    async function loadKPIs() {
        ...
        if (response.status===200) {
            kpis = response.data
            kpis = [
            ...kpis.filter((elem)=>{return elem.necessity==="mandatory"}),
            ...kpis.filter((elem)=>{return elem.necessity==="recommended"}),
            ...kpis.filter((elem)=>{return elem.necessity==="optional"})
            ]
        }
    }
</script>
    ...
    <form class="bg-white p-4 rounded-md w-1/2" on:submit={()=>{}}>
        <div class="form-control space-y-2">
            ...
            <label class="input-group">
                ...
                <input type="text" placeholder="Enter service name..." class="input input-bordered w-[80%]" bind:value={service}/>
            </label>
            <label class="input-group">
                <span class="w-[20%]">Abbreviation</span>
                <input type="text" placeholder="Enter service abbreviation..." class="input input-bordered w-[80%]" bind:value={abbreviation}/>
            </label>
            <label class="input-group">
            ...
            {#each kpis as kpi, i}
                ...
                {/if}
            {/each}
            <div class="divider">Optional KPIs <button disabled class="btn btn-ghost btn-xs" on:click={()=>{alert("Yeah!")}}><AddAlt/></button></div>
            {#each kpis as kpi}
