<script lang="ts">
	...
	import { indicators, measurements, steps } from '$lib/stores/consortia';
				...
				...kpis.filter((elem) => {
					return (
						elem.categories.find((c) => {
							return c.name === services[selectedService].category;
						})?.necessity === 'mandatory'
					);
				}),
				...kpis.filter((elem) => {
...
<div class="flex justify-center">
	<form class="bg-white p-4 rounded-md w-1/2" on:submit={sendMeasurement}>
		<div class="form-control space-y-2">
			...
			<div class="join">
				<label class="join-item w-[20%] bg-base-300 label"> Service </label>
				<select
			...
			<div class="join">
				<!-- svelte-ignore a11y-label-has-associated-control -->
				<label class="join-item w-[20%] bg-base-300 label"> Date </label>
				<input
					type="month"
					class="join-item input input-bordered w-[80%]"
					bind:value={measurementCollector.date}
				/>
			</div>
			{#if kpis}
				{#if services[selectedService].category === 'Consortia'}
					<UploadCard
						steps={$steps}
						on:finish={() => {
				...
				{:else}
					...
					{#each kpis as kpi, i}
						<div class="join">
							...
							<input
								type="number"
								class="join-item input input-bordered w-[75%]"
								bind:value={measurementCollector[kpi.name]}
							/>
							<!-- svelte-ignore a11y-click-events-have-key-events -->
							<label
								for="kpi-description"
								class="join-item btn btn-secondary w-[5%]"
								on:click={() => {
									clickedKPI = i;
								}}><Information /></label
							>
		...
		<label for="kpi-description" class="modal cursor-pointer">
			<label class="modal-box relative" for="">
				...
				<p class="italic text-sm">
					Necessity: {kpis[clickedKPI].categories.find((c) => {
						return c.name === services[selectedService].category;
					})?.necessity}
				</p>
