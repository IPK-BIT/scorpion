        ...
        <tbody>
            {#each dates as date}
                <tr class="hover text-center">
                    <td class="font-semibold bg-base-300">{new Date(date).toLocaleDateString("en", {year: "numeric", month: "short"})}</td>
                    {#each results[dates.findIndex((x)=>{return x===date})] as result}
                        <td>{result}</td>
                    {/each}
                    {#if removable}
                        <td>
                            <button class="btn btn-sm btn-ghost hover:btn-error" on:click={()=>{sendRemove(date)}}>Delete</button>
                        </td>
                    {/if}
                </tr>
            {/each}
