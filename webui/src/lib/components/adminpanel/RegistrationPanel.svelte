  ...
  async function answerRequest(request_id: string, decision: boolean) {
    const response = await axios.delete('/requests/registration', {
      ...
      params: {
        ...
        is_admin: $registration_requests.find((r)=>{return r.user_id===request_id})?.is_admin,
        accept: decision
      }
    })
    if (response.status===200) {
      if (decision) {
        let request = $registration_requests.find((r)=>{return r.user_id===request_id})
        ...
        $users = [...$users, {user_name: request.user_name, email: request.email, providers: [], is_admin: request.is_admin!=null?request.is_admin:false}]
      }
      $registration_requests = $registration_requests.filter((r)=>{return r.user_id!=request_id})
    }
  }
              ...
              <tbody>
                ...
                <tr>
                    ...
                    <th class="text-center">
                        <button class="btn btn-ghost hover:btn-success" on:click={()=>{answerRequest(request.user_id, true)}}>Accept</button>
                        <button class="btn btn-ghost hover:btn-error" on:click={()=>{answerRequest(request.user_id, false)}}>Decline</button>
                    </th>
                  </tr>
                {/each}
              </tbody>              
