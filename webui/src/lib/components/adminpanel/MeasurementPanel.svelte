...
<Card title="Measurement Deletion">
    <div slot="card-content" class="space-y-2">
        <div class="join w-full">
            <label for="serviceselect" class="join-item label bg-base-200 pl-4 w-1/6 border">Service</label>
            <select id="serviceselect" class="join-item select select-bordered w-5/6" bind:value={selected} on:change={loadKPIs}>
        ...
        {#if selected}
        <Table bind:kpis={measurementTable.kpis} bind:dates={measurementTable.dates} bind:results={measurementTable.results} removable on:remove={removeMeasurements}></Table>
        {/if}
    </div>
</Card>