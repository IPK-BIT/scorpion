<script lang="ts">
	...
	function drawChart() {
		...
		for (const measurement of measurements) {
			if (
				measurement.value &&
				(measurement.value > avg + cl_val * avg || measurement.value < avg - cl_val * avg)
			) {
				x_viol.push(measurement.date), y_viol.push(measurement.value);
			}
		}
	...
	<Card title="Report Card">
		<div slot="card-content" class="space-y-2">
			<div class="form-control flex flex-row space-x-4">
				<select
					class="select select-bordered w-4/6"
					bind:value={selected_service}
					on:change={loadKPI}
				>
			...
			{#if selected_service}
				<div class="form-control flex flex-row">
					<select
						class="select select-bordered w-4/6"
						bind:value={selected_indicator}
						on:change={loadMeasurements}
					>
						<option value="">Select an indicator</option>
						{#each indicators.filter((i) => {
							return i.selected;
						}) as indicator}
